...
This enables design-converter writers to execute Figma Plugin API code over HTTP.
...
Start it with: python3 -m design_converter.adapters.figma.bridge_server --port 9223
"""
...
class FigmaBridgeClient:
    ...
    def get_connection_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["server_running"]:
                ...
                result["suggestions"] = [
                    f"Start the bridge server: python3 -m design_converter.adapters.figma.bridge_server --port {self._port}",
            ...
            result["suggestions"] = [
                ...
                f"Start it with: python3 -m design_converter.adapters.figma.bridge_server --port {self._port}",
            ]
    ...
    def create_rectangle(
        ...
        *,
        ...
        parent_code = f"figma.getNodeById({json.dumps(parent_id)})" if parent_id else "figma.currentPage"
...
"""
    ...
    def create_frame(
        ...
        *,
        ...
        parent_code = f"figma.getNodeById({json.dumps(parent_id)})" if parent_id else "figma.currentPage"
        layout_code = f"n.layoutMode = {json.dumps(layout)};" if layout != "NONE" else ""
        name_js = json.dumps(name)
...
"""
    ...
    def create_text(
        ...
        *,
        ...
        font_name_js = json.dumps(font_name)
...
(async()=>{{
  ...
  parent.appendChild(n);
  await figma.loadFontAsync({{family: {font_name_js}, style: "Regular"}});
  n.characters = {content_js};
  n.fontSize = {font_size};
  n.fontName = {{family: {font_name_js}, style: "Regular"}};
  n.x = {x};
  n.y = {y};
  {fill_code}
  return {{success: true, nodeId: n.id}};
