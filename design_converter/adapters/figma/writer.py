...
from design_converter.adapters.base import BaseWriter
...
class _FigmaCodeEmitter:
    ...
    def _set_size(self, var: str, node: UNNode) -> None:
        ...
        """
        from design_converter.ir.nodes import SizingMode
    ...
    def _emit_variable_bindings(self, var: str, node: UNNode) -> None:
        """Emit setBoundVariable() for design token bindings on this node.

        The canonical IR binding shape is ``property/variable/tool_id``. Older
        Figma-reader output used ``variable_id/variable_name``. Accept both so
        generated code does not depend on which adapter produced the UNNode.
        """
        ...
        for prop_name, binding in bindings.items():
            bound_prop = prop_name or getattr(binding, "property", "")
            variable_id = (
                getattr(binding, "variable_id", None)
                or getattr(binding, "tool_id", None)
                or ""
            )
            variable_name = (
                getattr(binding, "variable_name", None)
                or getattr(binding, "variable", None)
                or variable_id
            )
            if variable_id.startswith("figma:"):
                variable_id = variable_id.split(":", 1)[1]

            safe_comment = str(variable_name).replace("*/", "* /")
            if not bound_prop or not variable_id:
                self._lines.append(
                    f"  /* token binding skipped: {safe_comment} has no Figma variable id */"
                )
                continue

            vid = json.dumps(str(variable_id))
            self._lines.append(
                f"  /* token: {safe_comment} */\n"
                f"  (function() {{\n"
                f"    try {{\n"
                f"      const _v = figma.variables.getVariableById({vid});\n"
                f"      if (_v) {var}.setBoundVariable({json.dumps(bound_prop)}, _v);\n"
                f"    }} catch(_e) {{ /* variable not found: {safe_comment} */ }}\n"
                f"  }})();"
            )
        ...
        if self._mode == "bridge":
            ...
            if not self._http_bridge.is_server_running():
                raise RuntimeError(
                    ...
                    f"  Start it with: python3 -m design_converter.adapters.figma.bridge_server --port {self._http_bridge_port}\n"
