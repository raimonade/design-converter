...
-----
    ...
    from design_converter.ir.nodes import UNNode
...
from design_converter.adapters.base import BaseWriter, WriteError
from design_converter.adapters.paper.client import PaperClient, PaperConnectionError, PaperToolError
...
class PaperWriter(BaseWriter):
    ...
    def write_node(
        ...
        2. Determine the target artboard / parent:
           - If ``replace_id`` is set, write into that node with
             mode="replace" (Paper replaces the target contents).
        ...
        if replace_id:
            # Paper's write_html replaces the target node's contents in this mode.
